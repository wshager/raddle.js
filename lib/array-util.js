...
function put$3(...$_a) {
	var $ = n.frame($_a);
	$ = $.array("array").item("position").item("member");
...
function foldLeft$4(...$_a) {
	var $ = n.frame($_a).array("array").item("zero").item("function").item("s");
	return ($ => {
...
function foldLeftAt$5(...$_a) {
	var $ = n.frame($_a).array("array").item("zero").item("function").item("at").item("s");
	return ($ => {
		if ($.test(n.eq($("s"), n.integer(0)))) {
			return $("zero");
		} else {
...
function firstIndexOf$2(...$_a) {
	var $ = n.frame($_a).array("array").item("lookup", n.zeroOrOne);
...
function lastIndexOf$2(...$_a) {
	...
	return foldRightAt($("array"), n.integer(0), function (...$_a) {
		...
		return ($ => {
			if ($.test($.test(n.eq($("pre"), n.integer(0))) && $.test(n.geq($("cur"), $("lookup"))))) {
				return $("at");
			} else {
				return $("pre");
			}
