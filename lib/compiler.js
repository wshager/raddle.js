...
const isCallNode = node => node && node.type == 14; // TODO detect AND/OR and convert to quotation
...
class Context {
	...
	addImport(length) {
		const ref = (prefix, loc) => {
			...
			// TODO merge properly
			compile((0, _l3n.fromStream)((0, _parser.parse)(loc)), this).subscribe(cx => {
				cx.apply().subscribe(() => {
	...
	addExport(length) {
		const ref = (qname, type, body) => {
			const { prefix, name } = (0, _compilerUtil.normalizeName)(qname);
	...
	getRef(qname) {
		...
		const { prefix, name } = (0, _compilerUtil.normalizeName)(qname, "n");
	...
	isBoundQname(qname) {
		const { prefix } = (0, _compilerUtil.prefixAndName)(qname);
...
const run = exports.run = (o, cx) => compile(o, cx).pipe((0, _operators.map)(cx => cx.apply()), (0, _operators.mergeAll)());