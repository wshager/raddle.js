...
class Context {
  ...
  addImport(length) {
    const ref = (prefix, ns, loc) => {
      ...
      const cx = this;
      return run("../raddled/" + loc + ".rdl")(cx).pipe((0, _operators.map)(() => {
...
const prepare = (core, prefix = "n", path = "../raddled/") => {
  ...
  return run(path + prefix + ".rdl")(cx).pipe((0, _operators.map)(() => cx));
};
...
const runnable = cx => (0, _rxjs.pipe)(_l3n.toVNodeStream, compile(cx), (0, _operators.switchMap)(cx => cx.apply()), (0, _operators.mergeMap)(x => (0, _rxjs.isObservable)(x) ? x : [x]));

const run = str => cx => runnable(cx)((0, _parser.parse)(str));
