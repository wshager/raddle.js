...
function filterNodesByName(nn,type,name){
    return n.filter(function(_){
        return _.get(0) === type && _.get(1) == name;
    });
}
...
export function stringJoin($seq,$sep) {
    ...
    console.log($seq.flatten().join(sep))
...
export function matches($str,$pat) {
    ...
    return seq(!!XRegExp.match(str,pat));
}
...
export function replace($str,$pat,$rep) {
	...
	let pat = XRegExp(atomic($pat),"g");
    let rep = atomic($rep);
	return XRegExp.replace(str,pat,rep);
}
