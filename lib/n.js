...
export function decimal($a){
    return seqOf(new Decimal(atomic($a)));
}
...
function eqf(a,b){
    if(a !== undefined) {
        if(a.constructor === Decimal) return a.equals(b);
        if(a.constructor === List && a._isNode) a = strFromNode(a).first();
    }
    if(b !== undefined && b.constructor === List && b._isNode) b = strFromNode(b).first();
    return a===b;
}
...
function select_3($nn,$path,$acc) {
	var $h = head($nn);
    if(atomic(and(eq(get($h,1),1),eq(get($h,2),$path)))) {
        $acc = seq($acc,$h);
    }
    if(gt(count($nn),1)) {
...
function selectAttribute_3($nn,$path,$acc) {
	var $h = head($nn);
    if(atomic(and(eq(get($h,1),2),eq(get($h,2),$path)))) {
        $acc = seq($acc,$h);
...
export function selectAttribute($node,$path) {
    ...
    if(path === "@*"){
	    ...
	    return selectAttribute_3(get(ret,3),$path,seq());
    }
}
/*
...
export function forEach($seq,$fn){
    var fn = atomic($fn);
    fn = typeof fn != "function" ? function($_0) {
        return fn;
    } : fn;
