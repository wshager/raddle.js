...
function handleBinOps(state, d) {
  ...
  for (let i = 0; i < state.bin[d]; i++) {
    ...
    if (!lastv || lastv > cur.v) {
      ...
      if (_triply.default.isBranch(cur)) {
      ...
      r.movePreviousSibling().moveLastChild() // insert the detached 2nd argument, add close paren and move after it, add a comma and move to the 1st argument
      .insertBefore(arg2).insertAfter(closeParen()).movePreviousSibling().insertBefore(comma()).movePreviousSibling() // insert opening paren and open with all siblings (= open + 2nd argument + comma)
...
function process(tpl, state) {
  const t = tpl.t,
        v = tpl.v,
        r = state.r;
  const d = state.depth; //console.log(t,v,tpl.o);
  ...
  if (t == 1) {
    ...
    * add(1,mult(2,3)) => pre, so next in last (we're in subs, so openBefore )
    ...
    if (v == 802 || v == 904 || v == 2003) {
      ...
      const isEmpty = x => x && _triply.default.nextSibling(_triply.default.firstChild(x)) == _triply.default.lastChild(x); //console.log(v,test,isEmpty(last));


      if (test && (simpleTypes.includes(test) && isEmpty(last) || complexTypes.includes(test) || kinds.includes(test))) {
        const nv = v + 3000;
        state.r.insertAfter(closeParen()).movePreviousSibling().insertBefore(openParen()).openBefore({
          t: 4,
          v: nv,
          o: opMap[nv]
        });
...
function charReducer(state, next) {
  ...
  let stop = qname && /[^a-zA-Z0-9\-_:]/.test(next) || number && /[^0-9.]/.test(next); //console.log("ct",char,type,state);
    ...
    $tpls.subscribe({
      next(tpl) {
        ...
        if (tpl.t === 0) {
          ...
          if (rdl) {
            (0, _richReducers.reduce)(r.traverse(), toRdl, $o);
          } else {
            (0, _richReducers.reduceAround)(r.traverse(), toL3, $o);
