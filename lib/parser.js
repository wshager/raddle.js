...
const log = r => (0, _richReducers.reduce)(_triply.default.traverse(r._root), (a, x) => {
  const n = _triply.default.view(x.$0 == 3 ? x.$3 : x, 1);
        ...
        if (test && complexTypes.includes(test)) {
          if (test == "function") {
            ...
            state.r.push(seq()).open(openParen()).push({
              t: 4,
              o: "rest-params"
            }).push(closeParen()).close().push(closeParen()).close().push({
              t: 4,
              o: "any"
            }).open(openParen()).push({
              t: 4,
